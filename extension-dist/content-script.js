...
(async function () {
  ...
  if (settings.errorCapture !== false) {
    installErrorCapture({
      throttleMs: settings.debugThrottleMs || 100
    });
  }
  ...
  // ---------------------------------------------------------------------------
  const [scriptsData, snippetsData] = await Promise.all([chrome.runtime.sendMessage({
    type: 'GET_SCRIPTS_FOR_URL',
    url: currentUrl
  }), chrome.runtime.sendMessage({
    type: 'GET_SNIPPETS_FOR_URL',
    url: currentUrl
  })]);
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
      endScripts.filter(s => s.metadata.runAt === 'document-end').forEach(s => injectUserScript(s));
    });
  } else {
    endScripts.filter(s => s.metadata.runAt === 'document-end').forEach(s => injectUserScript(s));
  }
  ...
  idleCallback(() => {
    endScripts.filter(s => s.metadata.runAt === 'document-idle' || !s.metadata.runAt).forEach(s => injectUserScript(s));
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    switch (message.type) {
      ...
      case 'RUN_SNIPPET_IN_PAGE':
        executeSnippetCode(message.code, message.name).then(result => sendResponse(result)).catch(err => sendResponse({
          success: false,
          error: err.message
        }));
        return true;
